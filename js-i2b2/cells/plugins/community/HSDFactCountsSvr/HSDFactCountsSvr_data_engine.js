...
i2b2.HSDFactCountsSvr.calcEngine = {
	data: {},
	ret: {},

	_resetOutput: function() {
		i2b2.HSDFactCountsSvr.calcEngine.ret = {
	...
	_getConfig: function() {
		...
		if (!i2b2.h.isBadObjPath('i2b2.HSDFactCountsSvr.model.options.ScaleY')) {
			switch(i2b2.HSDFactCountsSvr.model.options.ScaleY) {
				case "linear":
		...
		if (!i2b2.h.isBadObjPath('i2b2.HSDFactCountsSvr.model.options.BucketSizeAge')) {
	...
	generate: function(prsControl, prsExperiments) {
		...
		// get the mid-bucket offset for rendering
		this.data.midBucketOffset = ((this.data.ageMax - this.data.ageMin) / this.data.numOfBuckets) / 2;


		// build the datasets into a single array (index [0] being the control patientset)
		var datasets = [];
		datasets[0] = i2b2.h.XPath(prsControl.ajaxRet.statisticsXML, xpath_selector);
//		var maxVal = +i2b2.h.XPath(prsControl.ajaxRet.refXML, "//Summary/data/@FactsMax")[0].value; // be sure to convert from string to number
		for (var i=0; i<prsExperiments.length; i++) {
//			maxVal = Math.max(+i2b2.h.XPath(prsExperiments[i].ajaxRet.refXML, "//Summary/data/@FactsMax")[0].value, maxVal);
			datasets[i+1] = i2b2.h.XPath(prsExperiments[i].ajaxRet.statisticsXML, xpath_selector);
		}
		...
		// generate the isobar points
		for (var i=0; i<datasets[0].length; i++){
			dgroup.points.push([
				(+i2b2.h.XPath(datasets[0][i], "@AgeInYears")[0].value + this.data.midBucketOffset), 
				+i2b2.h.XPath(datasets[0][i], "@Facts5")[0].value
			]);
		}
		...
		for (var i=0; i<datasets[0].length; i++){
			dgroup.points.push([
				(+i2b2.h.XPath(datasets[0][i], "@AgeInYears")[0].value + this.data.midBucketOffset), 
				+i2b2.h.XPath(datasets[0][i], "@Facts10")[0].value
			]);
		}
		this.ret.datagroups.push(dgroup);
		...
		for (var i=0; i<datasets[0].length; i++){
			dgroup.points.push([
				(+i2b2.h.XPath(datasets[0][i], "@AgeInYears")[0].value + this.data.midBucketOffset), 
				+i2b2.h.XPath(datasets[0][i], "@Facts25")[0].value
			]);
		}
		this.ret.datagroups.push(dgroup);
		...
		for (var i=0; i<datasets[0].length; i++){
			dgroup.points.push([
				(+i2b2.h.XPath(datasets[0][i], "@AgeInYears")[0].value + this.data.midBucketOffset), 
				+i2b2.h.XPath(datasets[0][i], "@Facts50")[0].value
			]);
		}
		this.ret.datagroups.push(dgroup);
		...
		for (var i=0; i<datasets[0].length; i++){
			dgroup.points.push([
				(+i2b2.h.XPath(datasets[0][i], "@AgeInYears")[0].value + this.data.midBucketOffset), 
				+i2b2.h.XPath(datasets[0][i], "@Facts75")[0].value
			]);
		}
		this.ret.datagroups.push(dgroup);
		...
		for (var i=0; i<datasets[0].length; i++){
			dgroup.points.push([
				(+i2b2.h.XPath(datasets[0][i], "@AgeInYears")[0].value + this.data.midBucketOffset), 
				+i2b2.h.XPath(datasets[0][i], "@Facts90")[0].value
			]);
		}
		this.ret.datagroups.push(dgroup);
		...
		for (var i=0; i<datasets[0].length; i++){
			dgroup.points.push([
				(+i2b2.h.XPath(datasets[0][i], "@AgeInYears")[0].value + this.data.midBucketOffset), 
				+i2b2.h.XPath(datasets[0][i], "@Facts95")[0].value
			]);
		}
		this.ret.datagroups.push(dgroup);

		// do 3-point smoothing if needed
		if (i2b2.HSDFactCountsSvr.model.options.SmoothLines) {
		...
		for (var i=0; i<prsExperiments.length; i++) {
			var dgroup = {
				...
				"label": prsExperiments[i].sdxInfo.sdxDisplayName,
				"legend": "formal",
			...
			var temp_array = [];
			var pData = i2b2.h.XPath(prsExperiments[i].ajaxRet.statisticsXML, '//PatientList/data');
			for (var i1=0; i1<pData.length; i1++) {
				var temp = [];

				// get age [X position]
				var aData = i2b2.h.XPath(pData[i1], 'attribute::AgeInYears');
				try {
					temp[0] = +aData[0].value; // force conversion from string
				} catch(e) {
					temp[0] = null;
				}
				...
				var aData = i2b2.h.XPath(pData[i1], 'attribute::Facts');
				try {
					temp[1] = +aData[0].nodeValue; // force conversion from string
				} catch(e) {
					temp[1] = null;
				}
				...
				var aData = i2b2.h.XPath(pData[i1], 'attribute::PatientNum');
				try {
			...
			var aData = i2b2.h.XPath(prsExperiments[i].ajaxRet.statisticsXML, '//Summary/data/attribute::AgeInYearsAvg');
			try {
				var avgX = +aData[0].value;
			} catch(e) {
				var avgX = null;
			}
			var aData = i2b2.h.XPath(prsExperiments[i].ajaxRet.statisticsXML, '//Summary/data/attribute::FactsAvg');
			try {
				var avgY = +aData[0].value;
			} catch(e) {
				var avgY = null;
			}
			this.ret.datagroups.push({
				"id": "experimentMeanPoint"+i,
				"label": "MEAN of " + prsExperiments[i].sdxInfo.sdxDisplayName,
				"type": "meanPoint",
				"colorset": "Set1,7",
				"color": i,			// change this to [0-8] depending on which EXPERIMENT record
				"stroke": "2",
			...
			var aData = i2b2.h.XPath(prsExperiments[i].ajaxRet.statisticsXML, '//Summary/data/attribute::AgeInYearsStDev');
			try {
				var sdX = +aData[0].value;
			} catch(e) {
				var sdX = null;
			}
			var aData = i2b2.h.XPath(prsExperiments[i].ajaxRet.statisticsXML, '//Summary/data/attribute::FactsStDev');
			try {
				var sdY = +aData[0].value;
			} catch(e) {
				var sdY = null;
			}
			this.ret.datagroups.push({
				"id": "experimentSDEllipse"+i,
				"label": "SD of " + prsExperiments[i].sdxInfo.sdxDisplayName,
				"type": "ellipse",
				"colorset": "Set1,7",
				"color": i,			// change this to [0-8] depending on which EXPERIMENT record
				"stroke": "2",
	...
	generate2: function(inControl, inExperiment) {
		...
		var temp_array = [];
		var pData = i2b2.h.XPath(inControl.ajaxRet.statisticsXML, 'descendant::patient/param[@column]/text()/../..');
		for (var i1=0; i1<pData.length; i1++) {
			var temp = {};
		...
		// bucketize the Records according to radio button settings from the GUI
		i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized = i2b2.HSDFactCountsSvr.calcEngine.bucketizeData(temp_array, this.data.numOfBuckets, this.data.ageMin, this.data.ageMax);
		this.data.midBucketOffset = ((this.data.ageMax - this.data.ageMin) / this.data.numOfBuckets) / 2;
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCountsSvr.calcEngine.buildPercentileSeries(0.05, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCountsSvr.calcEngine.buildPercentileSeries(0.10, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCountsSvr.calcEngine.buildPercentileSeries(0.25, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "2",
			"stroke": "4",
			"points": i2b2.HSDFactCountsSvr.calcEngine.buildPercentileSeries(0.5, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCountsSvr.calcEngine.buildPercentileSeries(0.75, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCountsSvr.calcEngine.buildPercentileSeries(0.90, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCountsSvr.calcEngine.buildPercentileSeries(0.95, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		for (var i=0; i<inExperiment.length; i++) {
			...
			// points data format [x, y, radius, ID, tooltip]
			var temp_array = [];
			var pData = i2b2.h.XPath(inExperiment[i].ajaxRet.statisticsXML, 'descendant::patient/param[@column]/text()/../..');
			var avgX = 0;
			var avgY = 0;
			for (var i1=0; i1<pData.length; i1++) {
				var temp = [];
...
//			var sdY = d3.deviation(temp_array, function(d) { return d[1]; });
	...
	buildPercentileSeries: function(percentile, startX, endX, bucketOffsetValue) {
		...
		for (var i=startX; i<endX; i++) {
			var idx1 = i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized.rangeQuantize(i);
			if (typeof ret[idx1] == "undefined") {
				// do a proper percentile calculation
				var cnt = i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized[idx1].length - 1;
				if (cnt >= 0) {
					var pidx = cnt * percentile;
					var pidxInt = parseInt(pidx);
					if (pidxInt == pidx) {
						ret[idx1] = i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized[idx1][pidx]*1;
					} else {
						// not a direct match, must interpolate the value
						switch (pidxInt) {
							//case 0:
							//case cnt:
								//ret[idx1] = i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized[idx1][pidxInt] * (pidx - pidxInt);
								//ret[idx1] = i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized[idx1][pidxInt] * (pidx - pidxInt);
								//break;
							default:
								ret[idx1] = ((i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized[idx1][pidxInt+1] - i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized[idx1][pidxInt]) * (pidx - pidxInt))*1 + i2b2.HSDFactCountsSvr.calcEngine.data.ageBucketized[idx1][pidxInt]*1;
								break;
						}
					}
				}
...
/* ============================ REFACTOR =========================== */	
i2b2.HSDFactCountsSvr.calcEngine.FactCountCalcServer = {
...
// ===============================================================================
// calculate an age distribution based on server-side precalculation
// ===============================================================================
i2b2.HSDFactCountsSvr.calcEngine.FactCountCalcServer = {
	bucketizeDataAuto: function(dataset, config) {
		console.dir(config);
		console.dir(dataset);
		...
		ret.rangedata = {
			value_func: config.funcExtractor,
			bucket_count: +config.bucketCnt,
			ranges: [],
			min: +config.min,
			max: +config.max
		}
		...
		for (var i=0; i<ret.rangedata.bucket_count; i++) {
			...
			if (typeof config.funcValueExtractor === 'undefined') {
				ret.buckets[i] = [];
			} else {
				ret.buckets[i] = 0;
			}
		...
		dataset.forEach(function(d) {
			var i = ret.rangedata.ranges.indexOf(ret.rangedata.quantizer(config.funcExtractor(d)));
			ret.buckets[i] = +config.funcValueExtractor(d) + ret.buckets[i];
		});

		// Flag datatype
		ret.PreCalulated = true;
...
// calculate an age distribution based on server-side precalculation
// ===============================================================================
i2b2.HSDFactCountsSvr.calcEngine.AgeCalcServer = {
...
// TODO ==> i2b2.h.XPath(i2b2.HSDFactCountsSvr.model.serverSide, '//Summary/data/@AgeInYearsMin')[0].value;
		...
		for (var i=0; i<ret.rangedata.bucket_count; i++) {
			ret.rangedata.ranges.push((((ret.rangedata.max - ret.rangedata.min)/ret.rangedata.bucket_count)*i)+ret.rangedata.min);
			if (typeof configuration.funcValueExtractor === 'undefined') {
				ret.buckets[i] = [];
			} else {
				ret.buckets[i] = 0;
			}
		...
		arRecords.forEach(function(d) {
			if (typeof configuration.funcValueExtractor === 'undefined') {
				...
				ret.buckets[ret.rangedata.ranges.indexOf(ret.rangedata.quantizer(configuration.funcExtractor(d)))] = configuration.funcValueExtractor(d);
			}
		});

		// Flag datatype
		if (typeof configuration.funcValueExtractor !== 'undefined') ret.PreCalulated = true;
