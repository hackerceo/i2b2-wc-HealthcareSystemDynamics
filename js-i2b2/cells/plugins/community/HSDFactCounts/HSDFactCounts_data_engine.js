...
i2b2.HSDFactCounts.calcEngine = {
	data: {},
	ret: {},

	_resetOutput: function() {
		i2b2.HSDFactCounts.calcEngine.ret = {
	...
	_getConfig: function() {
		...
		if (!i2b2.h.isBadObjPath('i2b2.HSDFactCounts.model.options.ScaleY')) { 
			switch(i2b2.HSDFactCounts.model.options.ScaleY) {
				case "linear":
		...
		if (!i2b2.h.isBadObjPath('i2b2.HSDFactCounts.model.options.BucketSizeAge')) { 
	...
	generate2: function(inControl, inExperiment) {
		...
		// bucketize the Records according to radio button settings from the GUI
		i2b2.HSDFactCounts.calcEngine.data.ageBucketized = i2b2.HSDFactCounts.calcEngine.bucketizeData(temp_array, this.data.numOfBuckets, this.data.ageMin, this.data.ageMax);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCounts.calcEngine.buildPercentileSeries(0.05, 1,100, this.data.midBucketOffset)
		};
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCounts.calcEngine.buildPercentileSeries(0.10, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCounts.calcEngine.buildPercentileSeries(0.25, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "2",
			"stroke": "4",
			"points": i2b2.HSDFactCounts.calcEngine.buildPercentileSeries(0.5, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCounts.calcEngine.buildPercentileSeries(0.75, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCounts.calcEngine.buildPercentileSeries(0.90, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);
		...
		var dgroup = {
			...
			"color": "1",
			"stroke": "2",
			"points": i2b2.HSDFactCounts.calcEngine.buildPercentileSeries(0.95, 1,100, this.data.midBucketOffset)
		};
		this.ret.datagroups.push(dgroup);

		if (i2b2.HSDFactCounts.model.options.SmoothLines) {
...
//			var sdY = d3.deviation(temp_array, function(d) { return d[1]; });
	...
	buildPercentileSeries: function(percentile, startX, endX, bucketOffsetValue) {
		...
		for (var i=startX; i<endX; i++) {
			var idx1 = i2b2.HSDFactCounts.calcEngine.data.ageBucketized.rangeQuantize(i);
			if (typeof ret[idx1] == "undefined") {
				// do a proper percentile calculation
				var cnt = i2b2.HSDFactCounts.calcEngine.data.ageBucketized[idx1].length - 1;
				if (cnt >= 0) {
					var pidx = cnt * percentile;
					var pidxInt = parseInt(pidx);
					if (pidxInt == pidx) {
						ret[idx1] = i2b2.HSDFactCounts.calcEngine.data.ageBucketized[idx1][pidx]*1;
					} else {
						// not a direct match, must interpolate the value
						switch (pidxInt) {
							//case 0:
							//case cnt:
								//ret[idx1] = i2b2.HSDFactCounts.calcEngine.data.ageBucketized[idx1][pidxInt] * (pidx - pidxInt);
								//ret[idx1] = i2b2.HSDFactCounts.calcEngine.data.ageBucketized[idx1][pidxInt] * (pidx - pidxInt);
								//break;
							default:
								ret[idx1] = ((i2b2.HSDFactCounts.calcEngine.data.ageBucketized[idx1][pidxInt+1] - i2b2.HSDFactCounts.calcEngine.data.ageBucketized[idx1][pidxInt]) * (pidx - pidxInt))*1 + i2b2.HSDFactCounts.calcEngine.data.ageBucketized[idx1][pidxInt]*1;
								break;
						}
					}
				}
